...
const GRADING_MODES: { key: GradingDepth; label: string; desc: string; icon: React.ReactNode }[] = [
  ...
  { key: 'careful', label: 'Chấm cẩn thận', desc: '~8-12s', icon: <FiEye className="w-3.5 h-3.5" /> },
  { key: 'thorough', label: 'Chấm kĩ càng', desc: '~15-25s', icon: <FiSearch className="w-3.5 h-3.5" /> },
]

const GradingDepthDropdown: React.FC<GradingDepthDropdownProps> = ({ isGrading, onGrade, label = 'Chấm điểm', dropUp = false }) => {
  ...
  const baseStyle = isGrading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-brand-cyan/30'
  ...
  return (
    <div className="relative" ref={containerRef}>
      <div className="flex">
        ...
        >
          <FiChevronDown className={`w-3.5 h-3.5 transition-transform ${dropUp ? (showMenu ? '' : 'rotate-180') : (showMenu ? 'rotate-180' : '')}`} />
        </button>
      </div>
