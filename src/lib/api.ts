...
class ApiClient {
  ...
  private async handleHttpError(
    ...
    throw new Error(errorData.error?.message || errorData.message || `HTTP ${status} error`)
  }
  ...
  async signup(email: string, password: string, displayName?: string) {
    ...
    return this.request('/api/auth/signup', {
      method: 'POST',
      body: JSON.stringify({ email: normalizedEmail, password: trimmedPassword, displayName }),
    })
  }
  ...
  // Moderation
  async getPublicModerationKeywords() {
    return this.request<string[]>('/api/moderation/keywords', { suppressAuthToast: true })
  }
}
